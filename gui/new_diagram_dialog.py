...
from PySide6.QtWidgets import QDialog, QGridLayout, QLabel, QHBoxLayout
from PySide6.QtWidgets import QLineEdit, QSizePolicy, QSpacerItem
    ...
    def __init__(self, parent, n):
        ...
        for i in range(n):
            ...
            labelNodeName.setText(
                QCoreApplication.translate("Dialog", self.names[i], None)
            )
            ...
            self.name_lines_edit[i].setText(
                QCoreApplication.translate("Dialog", self.names[i], None)
            )

            self.horizontalSpacer = QSpacerItem(
                15, 10, QSizePolicy.Expanding, QSizePolicy.Minimum
            )
            ...
            for j in range(n):
                ...
                if j < i:
                    ...
                    self.gridLayout.addWidget(
                        self.matrix_lines[-1][0], i + 1, j + 2, 1, 1
                    )

                    self.matrix_lines[-1][0].setText(
                        QCoreApplication.translate(
                            "Dialog", str(self.matrix[i][j]), None
                        )
                    )
