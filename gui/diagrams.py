...
from PySide6.QtWidgets import QGraphicsScale, QGraphicsScene
from PySide6.QtWidgets import QGraphicsView, QMenu, QSizePolicy
import config
import project
...
class GraphicsSceneBubbleChart(QGraphicsScale):
    def __init__(self):
    ...
    def positionCheck(self, x, y, double, button):
        ...
        name = ""
        # clearLine()
        if len(config.STUDENTS_LIST) > 1 and len(config.RESULT_MATRIX) == len(
            ...
            if name != "":
                hit = selectNode(name, double, button)
            else:
    ...
    def drawDiagram(self):
        ...
        if len(config.STUDENTS_LIST) > 1 and len(config.RESULT_MATRIX) == len(
            ...
            if self.width() > self.height():
                minNodeRadius = self.height() / (len(config.STUDENTS_LIST) + 2)
            else:
                minNodeRadius = self.width() / (len(config.STUDENTS_LIST) + 2)
            ...
            step = 0
            if (
                MrPlagiarism[-1][1] >= config.RANGE_PLAG
                and MrPlagiarism[0][1] >= MrPlagiarism[-1][1]
            ):
                step = int((MrPlagiarism[0][1] - MrPlagiarism[-1][1]) / 10) + 1
            elif (
                MrPlagiarism[-1][1] < config.RANGE_PLAG
                and MrPlagiarism[0][1] >= config.RANGE_PLAG
            ):
            ...
            for i in range(len(config.STUDENTS_LIST)):
                if MrPlagiarism[i][1] > 0:
                    nodeWidth = nodeHeight = (
                        minNodeRadius * MrPlagiarism[i][1] / 18
                    )  # 20?
                    if step < len(config.STUDENTS_LIST) / 2:
                        nodeWidth = nodeHeight = minNodeRadius * MrPlagiarism[i][1] / 14
                    if step < len(config.STUDENTS_LIST) / 4:
                        nodeWidth = nodeHeight = minNodeRadius * MrPlagiarism[i][1] / 12
                else:
                    nodeWidth = nodeHeight = minNodeRadius
            ...
            for i in range(len(config.STUDENTS_LIST)):
                ...
                if config.CHESS:
                    if i % 2 == 0:
                        red, green, blue, a = (95, 160, 200, 220)
                    else:
                        red, green, blue, a = (245, 145, 130, 220)
                ...
                if config.SELECTED_STUDENT != "":
                    if config.WAITING_FOR_ == self.Nodes[i].name:
                        ...
                        red, green, blue, a = (102, 151, 180, 150)

                    if (
                        config.STUDENTS_LIST[MrPlagiarism[i][2]]
                        in config.SELECTED_STUDENTS
                    ):
                ...
                if MrPlagiarism[i][1] >= config.RANGE_PLAG and (
                    MrPlagiarism[i][1] > 0 or config.SHOW_LINKLESS
                ):
                    if MrPlagiarism[i][1] < stage:
                        nodeBias = step - (
                            int(MrPlagiarism[0][1] / 10) - int(MrPlagiarism[i][1] / 10)
                        )
                    ...
                    if step > 0:
                        nodeY = (
                            self.height()
                            - pH * 2
                            - level * (int(MrPlagiarism[i][1] / 10)) / 2
                            - self.Nodes[i].height / 2
                            - dy
                        )
            ...
            for i in range(len(self.Nodes)):
                ...
                if (
                    MrPlagiarism[i][1] >= config.RANGE_PLAG
                    and step > 0
                    and (MrPlagiarism[i][1] > 0 or config.SHOW_LINKLESS)
                ):
                    ...
                    if config.SELECTED_STUDENT != "":
                        if config.WAITING_FOR_ == self.Nodes[i].name:
                            ...
                            redt, greent, bluet, at = (102, 151, 180, 255)
                    else:
                        redt, greent, bluet, at = (250, 240, 240, 255)
                    ...
                    if config.SHOW_NAMES:
                        ...
                        for j in range(len(textLevel)):
                            if (
                                abs(textX - textLevel[j][1]) < self.Nodes[i].width
                                and abs(textY - textLevel[j][0]) < font
                            ):
                                textY = textY + font * 2
                        ...
                        if textX < indent:
                            textX = indent
                        if textX + len(student) * (font - 3) > self.width():
                            textX = self.width() - len(student) * (font - 3) - indent

                        if textY < indent:
                            textY = indent
                        if textY + font > self.height():
                            textY = self.height() - font - indent
            ...
            if len(config.STUDENTS_LIST) > 1:
                ...
                for i in range(len(self.listNodes)):
                    ...
                    for t in range(10):
                        ...
                        if (
                            config.RANGE_PLAG < maxPlagList[i][1]
                            or config.SHOW_LINKLESS
                            or config.SELECTED_STUDENT == self.listNodes[i][t].name
                        ):
                            for j in range(len(self.listNodes)):
                                if (
                                    i != j
                                    and float(config.RESULT_MATRIX[i][j]) > 0
                                    and t < float(config.RESULT_MATRIX[i][j]) / 10
                                    and config.STUDENTS_LIST[j] not in notedList
                                ):

                                    penWidth = self.listNodes[i][t].width / 3

                                    x2 = (
                                        self.listNodes[j][t].x
                                        + self.listNodes[i][t].width / 2
                                    )
                                    y2 = (
                                        self.listNodes[j][t].y
                                        + self.listNodes[i][t].width / 2
                                    )
                                    ...
                                    if (
                                        float(config.RESULT_MATRIX[i][j])
                                        < config.RANGE_PLAG
                                    ):
                                    ...
                                    if config.CHESS:
                                        if i % 2 == 0:
                                            redl, greenl, bluel, al = (95, 160, 200, al)
                                        else:
                                            redl, greenl, bluel, al = (
                                                245,
                                                145,
                                                130,
                                                al,
                                    ...
                                    if config.SELECTED_STUDENT != "":
                                        if (
                                            config.SELECTED_STUDENT
                                            == self.listNodes[i][t].name
                                            and self.listNodes[j][t].name
                                            in config.SELECTED_STUDENTS
                                        ) or (
                                            config.SELECTED_STUDENT
                                            == self.listNodes[j][t].name
                                            and self.listNodes[i][t].name
                                            in config.SELECTED_STUDENTS
                                        ):
                                            redl, greenl, bluel, al = (
                                                226,
                                    ...
                                    if len(config.HIDED_STUDENTS) > 0:
                                        if (
                                            self.listNodes[i][t].name
                                            in config.HIDED_STUDENTS
                                            or self.listNodes[j][t].name
                                            in config.HIDED_STUDENTS
                                        ):
                                            al = 5

                                    if config.RANGE_PLAG > maxPlagList[j][1]:
                                        al = 5
                                    if (
                                        config.RANGE_PLAG > maxPlagList[j][1]
                                        and not config.SHOW_LINKLESS
                                    ):
                                        al = 0
                                    if (
                                        config.RANGE_PLAG > maxPlagList[j][1]
                                        and not config.SHOW_LINKLESS
                                        and config.SELECTED_STUDENT
                                        == self.listNodes[j][t].name
                                    ):
                                        al = 15
                                    ...
                                    path1.moveTo(
                                        x1 + self.listNodes[i][t].width / 4,
                                        y1 + self.listNodes[i][t].width / 4,
                                    )
                                    path2.moveTo(x1, y1)
                                    ...
                                    path1.cubicTo(
                                        ctrlX1, ctrlY1, ctrlX2, ctrlY2, x2a, y2a
                                    )
                                    path1.lineTo(x2, y2)

                                    path2.moveTo(x2a, y2a)
                                    path2.lineTo(
                                        x2 + self.listNodes[i][t].width / 4,
                                        y2 + self.listNodes[i][t].width / 4,
                                    )
                            ...
                            if config.CHESS:
                                if i % 2 == 0:
                                    red, green, blue, a = (95, 160, 200, a)
                                else:
                                    red, green, blue, a = (245, 145, 130, a)

                            if config.SELECTED_STUDENT != "":
                                if config.WAITING_FOR_ == self.listNodes[i][t].name:
                                    red, green, blue, a = (234, 255, 239, a)
                                elif (
                                    config.SELECTED_STUDENT == self.listNodes[i][t].name
                                ):
                                    ...
                                    red, green, blue, a = (102, 151, 180, a)
                            ...
                            self.addEllipse(
                                self.listNodes[i][t].x,
                                self.listNodes[i][t].y,
                                self.listNodes[i][t].width,
                                self.listNodes[i][t].height,
                                QPen(colorPen),
                                QBrush(colorBrush),
                            )
                ...
                for i in range(len(self.listNodes)):
                    ...
                    textColor = QColor(redt, greent, bluet, at)
                    if config.SHOW_NAMES and (
                        config.RANGE_PLAG < maxPlagList[i][1]
                        or config.SHOW_LINKLESS
                        or config.SELECTED_STUDENT == self.listNodes[i][0].name
                    ):
    ...
    def drawDiagram(self):
        ...
        if len(config.STUDENTS_LIST) > 1 and len(config.RESULT_MATRIX) == len(
            ...
            if len(config.STUDENTS_LIST) > 1:
                for i in range(len(config.STUDENTS_LIST)):
                    self.Nodes.append(
                        Node(config.STUDENTS_LIST[i], nodeWidth, nodeHeight)
                    )
                    angle = i * theta
                    dx = int(radius * math.cos(angle))
                    dy = int(radius * math.sin(angle))
                ...
                for i in range(len(self.Nodes)):
                    ...
                    for j in range(len(self.Nodes)):
                        if (
                            ...
                            if config.CHESS:
                                if i % 2 == 0:
                                    red, green, blue = (95, 160, 200)
                                else:
                                    red, green, blue = (245, 145, 130)

                            if config.SELECTED_STUDENT != "":
                                if (
                                    config.SELECTED_STUDENT == self.Nodes[i].name
                                    and self.Nodes[j].name in config.SELECTED_STUDENTS
                                ) or (
                                    config.SELECTED_STUDENT == self.Nodes[j].name
                                    and self.Nodes[i].name in config.SELECTED_STUDENTS
                                ):
                                    red, green, blue, a = (226, 113, 105, 255)
                                else:
                                    red, green, blue, a = (102, 151, 180, 20)

                            # scene.addLine(x1, y1, x2, y2, QPen(color, penWidth))
                            if len(config.HIDED_STUDENTS) > 0:
                                if (
                                    self.Nodes[i].name in config.HIDED_STUDENTS
                                    or self.Nodes[j].name in config.HIDED_STUDENTS
                                ):
                                    a = 5
                ...
                for i in range(len(self.Nodes)):
                    ...
                    if (
                        config.RANGE_PLAG < maxPlagList[i][1]
                        or config.SHOW_LINKLESS
                        or config.SELECTED_STUDENT == self.Nodes[i].name
                    ):
                        if config.CHESS:
                            if i % 2 == 0:
                                red, green, blue = (95, 160, 200)
                        ...
                        if config.SELECTED_STUDENT != "":
                            if config.WAITING_FOR_ == self.Nodes[i].name:
                                ...
                                red, green, blue = redt, greent, bluet = (102, 151, 180)
                        ...
                        self.addEllipse(
                            self.Nodes[i].x - self.Nodes[i].width / 2,
                            self.Nodes[i].y - self.Nodes[i].height / 2,
                            self.Nodes[i].width,
                            self.Nodes[i].height,
                            pen,
                            brush,
                        ...
                        if config.SHOW_NAMES:
                            ...
                            if circleCenterX - textX - self.Nodes[i].width >= 0:
                                ...
                                textX = textX + self.Nodes[i].width / 2

                            if circleCenterY - textY >= 0:
                                textY = (
                                    textY
                                    - self.Nodes[i].width
                                    - self.Nodes[i].width / 3
                                )
                            else:
                                textY = textY + self.Nodes[i].width / 3
                            ...
                            if textX + len(student) * (font - 3) > self.width():
                                textX = (
                                    self.width() - len(student) * (font - 3) - indent
                                )
    ...
    def contextMenuEvent(self, event):
        if config.SELECTED_STUDENT != "":
            ...
            if len(config.SELECTED_STUDENTS) > 0:
                ...
                compare_iconPath = f"{appPath()}/icons/compare.png"
