...
class LinesView(QGraphicsView):
    def __init__(self, parent):
        ...
        self.viewRect = QRectF(0.0, 0.0, float(self.width() - 10), float(self.height() - 10))
...
class LinesScene(QGraphicsScene):
            ...
            if not hit:
                for i in range(len(self.cell)):
                    for j in range(len(self.cell)):
                        self.selectedGlob = None
                        self.selectedTarget = None
                        if self.cell[i][j].occupied: self.cell[i][j].glob.deselect()
                        else: self.cell[i][j].untarget()

            for i in range(len(self.cell)):
                for j in range(len(self.cell)):
                    self.cell[i][j].setPath(False)
            if self.selectedTarget != None: self.moveGlog()
            else: self.drawLines()
        else:
            self.cell = []
            self.selectedGlob = None
        ...
        if self.selectedTarget != None and self.selectedGlob != None: 
            ...
            if find[0]: 
                ...
                if len(lines) >= self.Goal:
                    ...
                    for i in range(len(self.cell)):
                        for j in range(len(self.cell)):
                            if not self.cell[i][j].occupied: freeCells = freeCells + 1
                            self.cell[i][j].setPath(False)
    ...
    def setGlobs(self):
        ...
        for i in range(len(self.cell)):
            for j in range(len(self.cell)):
                if not self.cell[i][j].occupied: listFreeCells.append([i,j]) 
    ...
    def addNewGlobs(self, n):
        ...
        for i in range(n):
            ...
            if color == 2: globs.append(Glob(self.color_2, "color_2"))
            if color == 3: globs.append(Glob(self.color_3, "color_3"))
            if color == 4: globs.append(Glob(self.color_4, "color_4"))
            if color == 5: globs.append(Glob(self.color_5, "color_5"))
            if color == 6: globs.append(Glob(self.color_6, "color_6"))
    ...
    def drawLines(self):
        ...
        if self.GAME:
            for i in range(len(self.Prediction)):
                ...
                PredictionX, PredictionY = (x + (scale/10)*(i + 1) + scale*i, scale - scale/4)
                self.addEllipse(PredictionX + 1, PredictionY + 1, scale - scale/4, scale - scale/4, self.linePenDark, self.predictionShadow)
        ...
        for i in range(len(self.cell)):
            for j in range(len(self.cell)):
                if self.cell[i][j].occupied:
                    self.cell[i][j].glob.setCoordinates(scale, self.cell[i][j].x, self.cell[i][j].y)
                    
                    x, y = (self.cell[i][j].glob.x, self.cell[i][j].glob.y)
                    w, h = (self.cell[i][j].glob.diameter, self.cell[i][j].glob.diameter)
                    ...
                    if self.cell[i][j].target: self.addRect(x, y, w, h, self.targetPen, self.targetColor)
                    if self.cell[i][j].path: self.addRect(x, y, w, h, self.targetPen, self.pathColor)
...
class Cell():       
    ...
    def targetable(self): self.target = True
    def untarget(self): self.target = False
...
class Glob():
    ...
    def select(self): self.chosen = True
    def deselect(self): self.chosen = False
    def setInLine(self): self.inLine = True
...
def checkLines(field, theGlob, goal):
    ...
    if pos != (0,0): line_1.extend(lineDirection(pos, -1, -1, length, field, color))
    if pos != (length, length): line_1.extend(lineDirection(pos, 1, 1, length, field, color))

    if pos[0] != 0 and pos[1] != length: line_2.extend(lineDirection(pos, -1, 1, length, field, color))
    if pos[1] != 0 and pos[0] != length: line_2.extend(lineDirection(pos, 1, -1, length, field, color))

    if pos[1] != length: line_3.extend(lineDirection(pos, 0, 1, length, field, color))
    if pos[1] != 0: line_3.extend(lineDirection(pos, 0, -1, length, field, color))

    if pos[0] != length: line_4.extend(lineDirection(pos, 1, 0, length, field, color))
    if pos[0] != 0: line_4.extend(lineDirection(pos, -1, 0, length, field, color))

    if len(line_1) >= goal: lines.extend(line_1)
    if len(line_2) >= goal: lines.extend(line_2)
    if len(line_3) >= goal: lines.extend(line_3)
    if len(line_4) >= goal: lines.extend(line_4)
